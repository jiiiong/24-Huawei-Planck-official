...
from log import scheduler_logger, main_logger, robot_logger, logger, boat_logger
from core import Env
from core import Robot, Robot_Extended_Status
from core import Berth, Goods, Boat, Boat_Extended_Status
...
class Scheduler:
    ...
    def init_boats(self):
        for i, boat in enumerate(self.env.boatss):
            boat.env = self.env
        pass
    ...
    def boats_zhen_handler(self):
        boats = self.env.boats
        boat_logger.info("")
        boat_logger.info(f"zhen: {self.env.global_zhen}")
        # 下一帧更新上一帧结束后的boats状态的转换
        boat_logger.info(f"func: boats_zhen_handler > update extended status")
        for boat in boats:
            boat.update_extended_status()
    ...
    def schedule_boats(self):
        logger.info(f"At zhen: {self.env.global_zhen} schedule_boats")
        global step
        # boat0_count = 0
        ...
        for i in range(self.env.boat_num):
            ...
            # logger.info(f"FUNC: schedule_boats > self.env.boat_num: {self.env.boat_num}")

            # if len(self.env.boats) >= 3:
            #     if i == 1 and self.env.boats[2].status == 0 or self.env.boats[2].status == 2:
            #         boat2 = self.env.boats[2]
            #         if  boat2.extended_status == Boat_Extended_Status.Uninitialized:
            #             boat2.go_to_berth_from_purchase_point(0)
            #         elif boat2.extended_status == Boat_Extended_Status.OnBerth:
            #             # logger.info("Func: schedule_boats > boat1.extended_status == Boat_Extended_Status.OnBerth")
            #             boat2.go_to_delivery_point(0)
            #         elif boat2.extended_status == Boat_Extended_Status.OnDeliveryPoint:
            #             # logger.info("Func: schedule_boats > boat1.extended_status == Boat_Extended_Status.OnDeliveryPoint")
            #             boat2.go_to_berth_from_delivery_point(2)
            #
            #
            #         # if self.env.attrs_grid[self.env.boats[0].x][self.env.boats[0].y].is_mooring:
            #         #     print("berth", 0)
            #         elif boat2.actions_stack:
            #             action = boat2.actions_stack.pop(0)
            #             # logger.info(f"BOAT1 Pos: {self.env.boats[2].pos} Dir: {self.env.boats[2].dir} ACTION: {action}")
            #             if action == "ship":
            #                 print("ship", 2)
            #             elif action == "rot 0":
            #                 print("rot", 2, 0)
            ...
            if len(self.env.boats)>= 2:
                if i == 1 and (self.env.boats[1].status == 0 or self.env.boats[1].status == 2):
                    boat1 = self.env.boats[1]
                    if  boat1.extended_status == Boat_Extended_Status.Uninitialized:
                        boat1.go_to_berth_from_purchase_point(0)
                    elif boat1.extended_status == Boat_Extended_Status.OnBerth:
                        # logger.info("Func: schedule_boats > boat1.extended_status == Boat_Extended_Status.OnBerth")
                        boat1.go_to_delivery_point(0)
                    elif boat1.extended_status == Boat_Extended_Status.OnDeliveryPoint:
                        # logger.info("Func: schedule_boats > boat1.extended_status == Boat_Extended_Status.OnDeliveryPoint")
                        boat1.go_to_berth_from_delivery_point(2)


                    # if self.env.attrs_grid[self.env.boats[0].x][self.env.boats[0].y].is_mooring:
                    #     print("berth", 0)
                    elif boat1.actions_stack:
                        action = boat1.actions_stack[0]
                        logger.info(f"Zhen {self.env.global_zhen} BOAT1 Pos: {self.env.boats[1].pos} Dir: {self.env.boats[1].dir} ACTION: {action}")
                        if action == "ship":
                            print("ship", 1)
                            boat1.actions_stack.pop(0)
                        elif action == "rot 0":
                            print("rot", 1, 0)
                            boat1.actions_stack.pop(0)
                        elif action == "rot 1":
                            ...
                            # 查看目标港口是否有船在，如果有的话就暂时不执行berth指令
                            is_boat_on_berth = False
                            for boat in self.env.boats:
                                if boat.pos == self.env.berths[boat1.target_berth_id].pos:
                                    is_boat_on_berth = True
                            if not is_boat_on_berth:
                                print("berth", 1)
                                boat1.actions_stack.pop(0)



            if i == 0 and (self.env.boats[0].status == 0 or self.env.boats[0].status == 2):
                boat0 = self.env.boats[0]
                if  boat0.extended_status == Boat_Extended_Status.Uninitialized:
                    boat0.go_to_berth_from_purchase_point(0)
                elif boat0.extended_status == Boat_Extended_Status.OnBerth:
                    # logger.info("BOAT 0 Func: schedule_boats > boat0.extended_status == Boat_Extended_Status.OnBerth")
                    boat0.go_to_delivery_point(0)
                elif boat0.extended_status == Boat_Extended_Status.OnDeliveryPoint:
                    # logger.info("BOAT 0 Func: schedule_boats > boat0.extended_status == Boat_Extended_Status.OnDeliveryPoint")
                    boat0.go_to_berth_from_delivery_point(2)


                # if self.env.attrs_grid[self.env.boats[0].x][self.env.boats[0].y].is_mooring:
                #     print("berth", 0)
                if boat0.actions_stack:
                    action = boat0.actions_stack[0]
                    logger.info(f"Zhen {self.env.global_zhen} BOAT0 Pos: {self.env.boats[0].pos} Dir: {self.env.boats[0].dir} ACTION: {action}")
                    if action == "ship":
                        print("ship", 0)
                        boat0.actions_stack.pop(0)
                    elif action == "rot 0":
                        print("rot", 0, 0)
                        boat0.actions_stack.pop(0)
                    elif action == "rot 1":
                        ...
                        is_boat_on_berth = False
                        for boat in self.env.boats:
                            if boat.pos == self.env.berths[boat0.target_berth_id].pos:
                                is_boat_on_berth = True
                        if not is_boat_on_berth:
                            print("berth", 0)
                            boat0.actions_stack.pop(0)
                # elif step == 0:
                #     # logger.info("sizeof %d", len(self.env.route_between_each_berth[1][0]))
                #     self.env.boat0_actions = self.env.route_between_each_berth[1][0]
                #     step += 1
                # elif step == 1:
                #     # logger.info("len %d", len(self.env.route_from_berth_to_target[0][0]))
                #     self.env.boat0_actions = self.env.route_from_berth_to_target[0][0]
                #     step += 1
                # elif step == 2:
                #     self.env.boat0_actions = self.env.route_from_target_to_berth[0][2]
                #     step += 1



                # elif self.env.boats[0].pos == self.env.berths[0].pos:
                #     logger.info("len %d", len(self.env.route_from_berth_to_target[0][0]))
                #     self.env.boat0_actions = self.env.route_from_berth_to_target[0][0]
