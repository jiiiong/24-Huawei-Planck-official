...
if RELEASE == False:
    ...
    robot_logger.setLevel(logging.DEBUG)
    fh_for_robot_logger = logging.FileHandler("solution/log/route.log", mode="w+")
    fh_for_robot_logger.setFormatter(fmt)
    robot_logger.addHandler(fh_for_robot_logger)

    boat_logger.setLevel(logging.INFO)
    fh_for_boat_logger = logging.FileHandler("solution/log/boat.log", mode="w+")
    fh_for_boat_logger.setFormatter(fmt)
    robot_logger.addHandler(fh_for_boat_logger)
