...
from log import scheduler_logger, main_logger, robot_logger,berth_logger
...
class Scheduler:
    ...
    def scheduler_buy_add(self):
        ...
        if self.env.boat_num==1 and self.env.money>=8000 and self.env.global_zhen >6000:# self.env.robot_num >= (self.env.berth_num*3-1):
    ...
    def schedule_round_trip(self,boat_id:int,purchase_id:int ,route:List[int],delivery_sVec_id:int):
        ...
        # 如果船只在港口路线上
        if boat.sVec in berthList and len(boat.actions)==0:
            ...
            if boat.goods_num == self.env.boat_capacity:   
                ...
                # 当前港口货物装完了
                    # 但是目前的船只还有容量，前往下一个港口
                    if boat.goods_num<self.env.boat_capacity:
                        local_index = berthList.index(boat.sVec)
                        boat.ship_from_A_to_B(boat.sVec,berthList[((local_index+1)%len(berthList))])
                        berth_logger.info("boat_id:%s next_index:%s",boat_id,((local_index+1)%len(berthList)))
                    else:
                        boat.ship_from_A_to_B(boat.sVec,delivery_sVec)
                        berth_logger.info("go")
        # 如果船只在售卖点
        if boat.sVec == delivery_sVec and len(boat.actions)==0:
    ...
    def schedule_boats(self):
        ...
        half_length = len(berth_id_list) // 2
        boat_1_route = berth_id_list
        # if self.env.boat_num ==2:
        #     boat_1_route = berth_id_list[:half_length]
        #     boat_2_route = berth_id_list[half_length:]
            # berth_logger.info("boat_1_route: %s",boat_1_route)
            # berth_logger.info("boat_2_route: %s",boat_2_route)
        boat_1_route = berth_id_list[:half_length]
        boat_2_route = berth_id_list[half_length:]
        # boat_1_route = [0,1,3]
        # boat_2_route = [2,4]
        ...
        if self.env.boat_num!=0:
            ...
            purchase_id = 0
            self.schedule_round_trip(0,purchase_id,boat_1_route,delivery_id)
            if self.env.boat_num == 2:
                delivery_id = self.env.delivery_sVec_list.index(self.env.delivery_sVec_list[-1])
                purchase_id = 1
                self.schedule_round_trip(1,purchase_id,boat_2_route,delivery_id)
